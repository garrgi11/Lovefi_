...
# Initialize the test agent with a different port to avoid conflicts
test_agent = Agent(
    name="TestAgentAgentVerse", 
    seed="test_seed_agentverse_5679",
    port=8002,
...
async def handle_match_response(ctx: Context, sender: str, response: MatchResponse):
    ...
    if current_test_index < len(TEST_PROFILES):
        ...
        if current_test_index < len(TEST_PROFILES):
            ...
            # Using the AgentVerse agent address
            agentverse_dating_agent_address = "agent1qggn4c3fsl409wtevwhehxxlhcc47vxw4v9lm59x77gzur63e9xzvnap0vp"
            await ctx.send(agentverse_dating_agent_address, request)
...
def print_test_summary(ctx):
    ...
    ctx.logger.info("\n" + "=" * 60)
    ctx.logger.info("DATING MATCH AGENT TEST SUMMARY (AgentVerse)")
    ctx.logger.info("=" * 60)
...
async def send_test_requests(ctx: Context):
    ...
    ctx.logger.info("🚀 Starting Dating Match Agent Tests (AgentVerse)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--run-tests":
        ...
        print("🚀 Starting Dating Match Agent Communication Tests (AgentVerse)")
        print(f"TestAgent address: {test_agent.address}")
        print("Starting test agent on http://localhost:8002...")
        print("Connecting to AgentVerse agent: agent1qggn4c3fsl409wtevwhehxxlhcc47vxw4v9lm59x77gzur63e9xzvnap0vp")
        print("AgentVerse URL: https://agentverse.ai/inspect/?uri=http%3A//127.0.0.1%3A8000&address=agent1qggn4c3fsl409wtevwhehxxlhcc47vxw4v9lm59x77gzur63e9xzvnap0vp")
        print("-" * 60)
        test_agent.run()
    else:
        # Simple test - just create agents and show addresses
        print("Creating test agent for AgentVerse...")
        
        # Create the test agent
        test_agent_simple = Agent(name="TestAgentAgentVerse", seed="test_seed_agentverse_5679", metadata={"type": "test", "agent_type": "test"})
        print(f"TestAgent address: {test_agent_simple.address}")
        
        # AgentVerse dating match agent address
        agentverse_dating_agent_address = "agent1qggn4c3fsl409wtevwhehxxlhcc47vxw4v9lm59x77gzur63e9xzvnap0vp"
        print(f"AgentVerse DatingMatchAgent address: {agentverse_dating_agent_address}")
        ...
        print("USAGE:")
        print("=" * 60)
        print("1. Make sure the AgentVerse agent is running and accessible:")
        print("   URL: https://agentverse.ai/inspect/?uri=http%3A//127.0.0.1%3A8000&address=agent1qggn4c3fsl409wtevwhehxxlhcc47vxw4v9lm59x77gzur63e9xzvnap0vp")
        print("   Address: agent1qggn4c3fsl409wtevwhehxxlhcc47vxw4v9lm59x77gzur63e9xzvnap0vp")
        print("\n2. Then run the tests:")
        print("   python testing_dating_match_agent_agentverse.py --run-tests")
